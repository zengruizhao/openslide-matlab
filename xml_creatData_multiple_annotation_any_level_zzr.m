clear;clc;
addpath(genpath('./src'));
ImgPath= 'J:\skin_xml\Train\';
xmlPath= 'J:\skin_xml\Train\';
                ...
                for j=1:(width/idx):(w-width+1)
                    ...
                    if (stage == 'i') %||stage== 'o'
                        ...
                        if ((xPose + width - 1) < Width) && ((yPose + height - 1) < Height)
                            patch = openslide_read_region(slide, xmin + j - 1, ymin + i-1, width, height, 'level', 1);% x20
                            II=patch(:,:,2:4);
                            ...
                            if ~exist(JPath);mkdir(JPath);end
